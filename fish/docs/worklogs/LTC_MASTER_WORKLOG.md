...
## V0.134 — QM Gap Integration
- integrated **179 fish** and **26 invert** missing QM entries directly into `data/species/*.js`
- normalized new cards to the current 41-field schema used by v0.133 entries
- updated `data/fish.js` so newly integrated entries are appended automatically even when not manually listed in `FISH_ORDER`
- generated a dedicated integration report and a `changed_files/` drop-in folder for Claude merge review
- flagged specialty animals and taxonomy edge cases for human review in the report


## V0.134 — QM Gap Integration (final)
- integrated **187 fish** and **26 invert** entries from the QM gap set directly into `data/species/*.js`
- preserved meaningful male/female/pair additions where the source pack already split them out
- normalized all new cards to the current v0.133 field shape, including water / compatibility fields and the long-form note fields
- updated `data/fish.js` to append unlisted species automatically after the manual feature order
- generated `changed_files/` and `LTC_V0134_QM_GAP_INTEGRATION_REPORT.md` for Claude merge review


## v0.135 — QM Deep QA Sweep
- Took the direct QM gap integration build and QA-polished all 213 newly added entries rather than leaving them on category-level boilerplate.
- Rewrote narrative card fields around subtype-specific behavior, feeding, and buying guidance.
- Added a Claude bridge manifest (`data/qm_gap_inventory_bridge.json`) plus `docs/handoffs/LTC_V0135_CLAUDE_QM_BRIDGE.md`.
- Normalized new-entry `photoTitle` keys to scientific names to improve default image pulls.
- Flagged ambiguous taxonomy rows that still deserve a human decision.


## v0.136 Final QM sweep
- Re-audited all 213 QM-added entries for schema completeness and water/compatibility coverage.
- Retained and corrected the three previously questioned specialty rows after confirming trade/information support.
- Normalized problematic slash-based photoTitle fields on Earmuff Wrasse, Yellow Tip Hermit Crab, and Slipper Lobster.
- Updated bridge manifest metadata so Claude can merge inventory work against the corrected cards.
