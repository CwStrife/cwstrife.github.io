...
const APP_VERSION = 'v0.188-exp-3';
...
// v0.188-exp — NEW EXPERIMENTAL MODAL TEMPLATE
// Mirrors LTC_Staff_EditInPlace_Mockup_v2.html design pattern. Three
// viewer states layered on body class:
//   (no class)            = Customer
//   body.is-staff         = Staff display (set when state.staffMode)
//   body.is-edit-mode     = Staff edit mode (set when state.editMode)
// editMode is per-modal (resets when modal closes). It's only meaningful
// when staffMode is also true.
//
// Editable cards trigger the EXISTING staff edit popup handlers
// (staffEditPrice, staffEditTank, etc.). Inline editing is a Phase 2.
//
// Uses scoped selectors under .fish-modal-exp so the new CSS doesn't
// collide with the legacy modal's .modal-magazine-flow / .mag-* rules.
// The compatibility gauges, water params, and fish marker injection
// hooks all use the SAME class names as the legacy modal so the
// existing JS init code (injectFishMarkers, initGaugeWaterCanvas,
// initWaterParamsCanvas) just works.
function modalTemplate(item){
  ...
  const summary = (typeof summaryText === 'function') ? summaryText(item) : '';

  // Inventory (editable in edit mode) data
  const priceDisplay = item.price ? formatMoney(item.onSale && item.salePrice ? item.salePrice : item.price) : '—';
  const oldPriceDisplay = (item.onSale && item.salePrice && item.price) ? formatMoney(item.price) : '';
  const tankDisplay = item.tankCode || '—';
  const stockNumberDisplay = item.stockNumber || '—';
  const qty = displayQuantityValue(item.quantity);
  const stockLabel = displayStockSize(item.stockSize);
  ...
  // Hold info — staff sees full, customer sees redacted
  const isHeld = !!item.reserved;
  const holdName = item.reservedFor || '';
  const holdUntil = item.reservedUntil ? formatDateShort(item.reservedUntil) : '';
  const holdStaffText = isHeld
    ? (holdName ? `For <strong>${holdName}</strong>${holdUntil ? ` <span class="fmx-muted">· until ${holdUntil}</span>` : ''}` : 'Held (no customer attached)')
    : 'No active hold';

  // v0.188-exp-2 — Status badge in the Inventory header is now the
  // canonical Available/Held/Out of stock indicator visible to BOTH
  // customers and staff. The Hold/Reserve section box has become
  // staff-only and shows the customer name + until date.
  // Status badge for the inventory section header
  const statusBadgeText = item.quarantine ? 'Quarantine' : (item.reserved ? 'Held' : (item.inStock ? 'Available' : 'Out of stock'));
  const statusBadgeTone = item.quarantine ? 'amber' : (item.reserved ? 'rose' : (item.inStock ? 'green' : 'red'));

  return `
    <div class="modal-magazine-flow modal-hero-magazine fmx-shell">

      <!-- ════════════════════════════════════════════════════════════
           v0.188-exp STAFF BANNER
           Visible only when state.staffMode (body.is-staff). Contains
           the Enter/Exit edit mode toggle button on the right side.
           Same button slot, label swaps based on body.is-edit-mode.
           v0.188-exp-3 — initial label/subtitle reflects current
           state.editMode so re-renders (after edit) preserve mode.
           ════════════════════════════════════════════════════════════ -->
      <div class="fmx-banner">
        <span class="fmx-banner-icon">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg>
        </span>
        <div class="fmx-banner-text">
          <strong>STAFF MODE</strong>
          <span data-fmx-banner-subtitle>${state.editMode ? 'Edit in place — tap any field to update it.' : 'Customer is viewing this profile. Tap Enter edit mode to make changes.'}</span>
        </div>
        <button type="button" class="fmx-banner-toggle" data-fmx-edit-toggle>${state.editMode ? 'Exit edit mode' : 'Enter edit mode'}</button>
      </div>
      ...
      <div class="modal-photo-card modal-hero-media modal-hero-wide">
        <div class="modal-photo modal-photo-magazine" data-detail-photo="${item.id}">
          ...
          <div class="modal-photo-copy modal-photo-copy-magazine">
            ...
            ${summary ? `<p class="hero-summary-line">${summary}</p>` : ''}
      ...
      <!-- ════════════════════════════════════════════════════════════
           INVENTORY SECTION — visible to customers (read-only display)
           and staff. In edit mode, each card is clickable and triggers
           the existing staff edit popup handlers.
           ════════════════════════════════════════════════════════════ -->
      <p class="fmx-section-header">Inventory <span class="fmx-status-badge fmx-status-${statusBadgeTone}">${statusBadgeText}</span></p>
      <div class="fmx-stat-grid">
        <div class="fmx-stat-card fmx-editable" data-edit-action="staffEditPrice" data-edit-id="${item.id}">
          <span class="fmx-stat-label">Price</span>
          <strong class="fmx-stat-value fmx-stat-money">${oldPriceDisplay ? `<span class="fmx-old-price">${oldPriceDisplay}</span>` : ''}${priceDisplay}</strong>
        </div>
        <div class="fmx-stat-card fmx-editable" data-edit-action="staffEditQuantity" data-edit-id="${item.id}">
          <span class="fmx-stat-label">Quantity</span>
          <strong class="fmx-stat-value">${qty}</strong>
        </div>
        <div class="fmx-stat-card fmx-editable" data-edit-action="staffEditTank" data-edit-id="${item.id}">
          <span class="fmx-stat-label">Tank</span>
          <strong class="fmx-stat-value">${tankDisplay}</strong>
        </div>
        <div class="fmx-stat-card fmx-editable" data-edit-action="staffEditStockSize" data-edit-id="${item.id}">
          <span class="fmx-stat-label">Size / grade</span>
          <strong class="fmx-stat-value fmx-stat-small">${stockLabel || '—'}</strong>
        </div>
        <div class="fmx-stat-card fmx-editable" data-edit-action="staffEditStockNumber" data-edit-id="${item.id}">
          <span class="fmx-stat-label">Stock #</span>
          <strong class="fmx-stat-value fmx-stat-small">${stockNumberDisplay}</strong>
        </div>
        <div class="fmx-stat-card fmx-editable" data-edit-action="staffEditStockInfo" data-edit-id="${item.id}">
          <span class="fmx-stat-label">Arrived</span>
          <strong class="fmx-stat-value fmx-stat-small">${arrival || '—'}</strong>
        </div>
      </div>

      <!-- HOLD / RESERVE — v0.188-exp-2: STAFF ONLY. Customer
           communication is handled by the Available/Held/Out of stock
           badge in the Inventory section header above. This box stays
           in the same position in the modal flow but only renders for
           staff, showing the full customer name + until date. -->
      <div class="fmx-staff-only">
        <p class="fmx-section-header">Hold / reserve</p>
        <div class="fmx-hold-card fmx-editable" data-edit-action="staffEditHold" data-edit-id="${item.id}">
          <div class="fmx-hold-row">
            <span class="fmx-hold-chip ${isHeld ? 'is-held' : ''}">${isHeld ? 'HELD' : 'AVAILABLE'}</span>
            <span class="fmx-hold-info">${holdStaffText}</span>
          </div>
        </div>
      </div>

      <!-- STAFF NOTE — visible only to staff (any sub-mode) -->
      <div class="fmx-staff-only">
        <p class="fmx-section-header">Staff note</p>
        <div class="fmx-staff-note fmx-editable" data-edit-action="staffEditStaffNote" data-edit-id="${item.id}">
          <p>${staffNote || '<span class="fmx-empty">No staff note. Tap to add.</span>'}</p>
        </div>
      </div>

      <!-- STATUS ACTIONS — visible only in staff edit mode -->
      <div class="fmx-staff-edit-only">
        <p class="fmx-section-header">Status actions</p>
        <div class="fmx-status-actions">
          ${item.inStock ? `
            <button type="button" class="fmx-action-btn fmx-action-sold" onclick="event.stopPropagation();ltcFx.bubbles(this);staffMarkSold('${item.id}')">Mark sold</button>
            <button type="button" class="fmx-action-btn fmx-action-loss" onclick="event.stopPropagation();ltcFx.jelly(this);staffMarkDead('${item.id}')">Mark loss</button>
            <button type="button" class="fmx-action-btn fmx-action-quarantine" onclick="event.stopPropagation();ltcFx.jelly(this);staffQuarantine('${item.id}')">Quarantine</button>
            <button type="button" class="fmx-action-btn fmx-action-photo" onclick="event.stopPropagation();ltcFx.jelly(this);staffUploadPhoto('${item.id}')">+ Photo</button>
            <button type="button" class="fmx-action-btn" onclick="event.stopPropagation();ltcFx.jelly(this);showSaleHistory('${item.id}')">Sale history</button>
          ` : `
            <button type="button" class="fmx-action-btn fmx-action-sold" onclick="event.stopPropagation();ltcFx.bubbles(this);staffRestockFish('${item.id}')">+ Add back to stock</button>
            <button type="button" class="fmx-action-btn fmx-action-photo" onclick="event.stopPropagation();ltcFx.jelly(this);staffUploadPhoto('${item.id}')">+ Photo</button>
            <button type="button" class="fmx-action-btn" onclick="event.stopPropagation();ltcFx.jelly(this);showSaleHistory('${item.id}')">Sale history</button>
          `}
        </div>
      ...
      <!-- QUICK INFO STATSTRIP — customer-visible -->
      <p class="fmx-section-header">Quick info</p>
      ...
      <p class="fmx-section-header">Compatibility gauges</p>
      ...
      ${factStack ? `<p class="fmx-section-header">Quick facts</p><div class="mag-fact-stack">${factStack}</div>` : ''}

      ${overviewText ? `<p class="fmx-section-header">About this fish</p><div class="mag-overview"><p>${overviewText}</p></div>` : ''}
      ...
      <p class="fmx-section-header">Similar fish you might like</p>
...
// v0.188-exp — LEGACY modal template preserved for fallback. The
// new experimental modal lives below as modalTemplate(). If we need
// to revert, change openFishModal to call modalTemplateLegacy() and
// remove the .fish-modal-exp class from the modal element.
function modalTemplateLegacy(item){
  const [reefText, reefClass] = reefChip(item.coralRisk);
  const [careText, careClass] = careChip(item.careDifficulty);
  const [aggText, aggClass] = aggressionChip(item.aggression);
...
function openFishModal(id){
  ...
  if(!fish) return;
  // v0.188-exp-3 — editMode is per-modal. Reset only when opening a
  // DIFFERENT fish. If openFishModal is called with the SAME id (e.g.,
  // by the refresh-after-edit handler in the .fmx-editable click flow),
  // preserve the current edit mode so a single edit doesn't kick the
  // user out of edit mode. Without this, every edit would force them
  // to tap Enter edit mode again before making the next change.
  const sameFish = (state.selectedId === id);
  if(!sameFish) {
  ...
  const body = document.getElementById('fishModalBody');
  if(!body) return;
  // v0.188-exp — Calls the new experimental modalTemplate. The legacy
  // template is preserved as modalTemplateLegacy() for fallback.
  ...
  if(modal){
    ...
    // v0.188-exp — mark this modal as the experimental layout so
    // the new CSS rules under .fish-modal-exp apply.
    modal.classList.add('fish-modal-exp');
  }
  // v0.188-exp — sync body.is-staff with state.staffMode so the
  // banner + staff sections show/hide via CSS without re-rendering.
  document.body.classList.toggle('is-staff', !!state.staffMode);
  ...
  // V0.117 — More details collapse toggle
  ...
  // v0.188-exp — Banner Enter/Exit edit mode toggle. Single button
  // slot in the staff banner. Label and behavior swap based on
  // body.is-edit-mode. Editable cards become clickable in edit mode.
  body.querySelectorAll('[data-fmx-edit-toggle]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const editing = document.body.classList.toggle('is-edit-mode');
      state.editMode = editing;
      btn.textContent = editing ? 'Exit edit mode' : 'Enter edit mode';
      const subtitle = body.querySelector('[data-fmx-banner-subtitle]');
      if(subtitle) {
        subtitle.textContent = editing
          ? 'Edit in place — tap any field to update it.'
          : 'Customer is viewing this profile. Tap Enter edit mode to make changes.';
      }
    });
  });
  // v0.188-exp — Editable cards. Click in edit mode triggers the
  // existing staff edit popup handler whose function name is in
  // data-edit-action. Click outside edit mode is a no-op (the
  // affordance is also CSS-disabled).
  body.querySelectorAll('.fmx-editable').forEach(card => {
    card.addEventListener('click', (e) => {
      if(!state.editMode) return;
      e.stopPropagation();
      const action = card.dataset.editAction;
      const editId = card.dataset.editId;
      if(action && editId && typeof window[action] === 'function') {
        if(typeof ltcFx !== 'undefined' && ltcFx.jelly) ltcFx.jelly(card);
        // v0.188-exp-3 — install a one-shot listener for the input
        // modal close event. After the staff edit popup runs its
        // existing persist + render flow, we re-open the fish modal
        // so the new values appear immediately. Without this, the
        // editable cards stay showing stale data until the user
        // manually closes and reopens the fish modal.
        const onInputClose = function(){
          document.removeEventListener('ltc:input-modal-closed', onInputClose);
          // Wait one tick for the existing edit function's render() and
          // persistStaffEdits() to fully settle, then re-render the fish
          // modal body with fresh data from the now-updated FISH array.
          setTimeout(function(){
            if(state.selectedId === editId) {
              const stillOpen = document.getElementById('fishOverlay');
              if(stillOpen && stillOpen.classList.contains('show')) {
                openFishModal(editId);
              }
            }
          }, 60);
        };
        document.addEventListener('ltc:input-modal-closed', onInputClose);
        // Safety net — if the popup never fires the event for some
        // reason (e.g., user navigates away), remove the listener
        // after 5 minutes so it doesn't leak.
        setTimeout(function(){
          document.removeEventListener('ltc:input-modal-closed', onInputClose);
        }, 300000);
        window[action](editId);
      }
    });
  });
...
function closeFishModal(){
  ...
  if(modal){
    modal.classList.remove('mobile-safe');
    // v0.188-exp — clear experimental flag and editing state on close
    modal.classList.remove('fish-modal-exp');
  }
  // v0.188-exp — reset body edit-mode flag on close
  document.body.classList.remove('is-edit-mode');
  state.editMode = false;
  const closeBtn = document.getElementById('closeFishBtn');
...
state.editMode = false; // v0.188-exp — per-modal edit mode flag
if(!state.analytics) state.analytics = {};
...
function checkPin(){
  ...
  if(input.value === STAFF_PIN){
    ...
    document.body.classList.add('is-staff'); // v0.188-exp
    state.compareList = [];
    updateCompareBar();
    closeStaffLogin();
...
function exitStaffMode(){
  state.staffMode = false;
  state.editMode = false;                       // v0.188-exp
  document.body.classList.remove('is-staff');   // v0.188-exp
  document.body.classList.remove('is-edit-mode'); // v0.188-exp
  updateCompareBar();
