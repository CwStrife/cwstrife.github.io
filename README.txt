...
function updateViewToggleUI(){
  ...
  const detailedBtn = document.getElementById('viewToggleDetailed');
  const compactBtn = document.getElementById('viewToggleCompact');
  if(!wrap || !detailedBtn || !compactBtn) return;
...
function renderCategories(){
  ...
  requestAnimationFrame(updateCategoryRailUI);
}
...
function cardTemplate(item){
  ...
  const priceHtml = isEncy || !item.price ? '' : item.onSale ? `<div class="price-badge" style="background:linear-gradient(135deg,rgba(255,80,80,.15),rgba(200,40,40,.12));border-color:rgba(255,100,100,.25)"><span style="font-size:10px;text-decoration:line-through;color:rgba(255,255,255,.35);display:block">${formatMoney(item.price)}</span><span class="price-value" style="color:#ff8888">${formatMoney(item.salePrice)}</span><span style="font-size:8px;font-weight:800;color:#ff6666;display:block;margin-top:1px">${typeof T==="function"?T("sale"):"SALE"}</span></div>` : `<div class="price-badge"><span class="price-value">${formatMoney(item.price)}</span></div>`;
...
function renderCardsAndMeta(){
  ...
  if(bundleContainer){
    ...
    bundleContainer.innerHTML = bhtml ? `<div style="display:flex;gap:10px;overflow-x:auto;padding-bottom:8px;scrollbar-width:none">${bhtml}</div>` : '';
  }
...
window.addEventListener('resize', updateCategoryRailUI);
